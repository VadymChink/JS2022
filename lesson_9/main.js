...
for (const course of coursesArray) {
    ...
    wrap.innerHTML = `
<h2>${course.title}</h2>
<h3>Month duration: ${course.monthDuration}</h3>
<h3>Hour duration: ${course.hourDuration}</h3>
`
    wrap.appendChild(ul);
...
let pages = ['Main', 'Products', 'About us', 'Contacts']
...
for (const courseAndDuration of coursesAndDurationArray) {

    let div = document.createElement('div');

    div.innerHTML = `<h2>${courseAndDuration.title}</h2><h3>Month Duration: ${courseAndDuration.monthDuration}</h3>`;
...
for (const courseAndDuration of coursesAndDurationArray) {
    ...
    div.innerHTML = ` <h1 class='heading'>${courseAndDuration.title}</h1> <p class="description">${courseAndDuration.monthDuration}</p>`;
    document.body.appendChild(div);
...
// з інпута та перевірити вік чи менше він ніж 18, та повідомити про це користувача;
